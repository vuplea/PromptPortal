...
describe('credentials', () => {
  test('accepts the right username and password', () => {
    expect(new Auth(PASSWORD).check(GOOD, '1.2.3.4')).toEqual({ ok: true });
  });
  ...
  test('accepts the bare password on the workstation gate', () => {
    const auth = new Auth(PASSWORD);
    expect(auth.checkPassword(Buffer.from(PASSWORD), '1.2.3.4')).toEqual({ ok: true });
...
describe('lockout', () => {
  ...
  test('browser and workstation lockouts are tracked separately', () => {
    ...
    expect(auth.checkPassword(Buffer.from(PASSWORD), '9.9.9.9')).toEqual({ ok: false, status: 429, retryAfter: 30 });
    expect(auth.check(GOOD, '9.9.9.9')).toEqual({ ok: true });
  });
  ...
  test('an empty workstation password never accumulates failures', () => {
    ...
    expect(auth.checkPassword(Buffer.from(PASSWORD), '9.9.9.9')).toEqual({ ok: true });
  });
});
